#!/usr/bin/env python3
"""
Hop Support v3 — Compute support counts WITH loop/non-loop separation.

Execludes 1-hop self loop (r1,r2 and x=y or y=z)
...
def build_loop_nonloop_count_query(r1: str, r2_pids: List[str]) -> str:
    ...
    return f"""
SELECT ?r2 ?is_loop (COUNT(*) AS ?support) WHERE {{
  {{
    SELECT DISTINCT ?r2 ?is_loop ?x ?y ?z WHERE {{
      VALUES ?r2 {{ {values} }}
      ?x wdt:{r1} ?y .
      ?y ?r2 ?z .
      FILTER(?x != ?y)
...
def build_loop_count_query(r1: str, r2_pids: List[str]) -> str:
    """Count only 2-hop return paths (x = z), excluding 1-hop self-loops."""
    values = " ".join(f"wdt:{pid}" for pid in r2_pids)
    return f"""
SELECT ?r2 (COUNT(*) AS ?support) WHERE {{
  {{
    SELECT DISTINCT ?r2 ?x ?y ?z WHERE {{
      VALUES ?r2 {{ {values} }}
      ?x wdt:{r1} ?y .
      ?y ?r2 ?z .
      FILTER(?x = ?z)
...
def triplets_writer_worker(
    output_path: str,
    out_queue: "queue.Queue[Optional[Dict[str, Any]]]",
    logger: logging.Logger,
) -> None:
    """Write streamed triplet-like support rows directly to JSONL."""
    while True:
        item = out_queue.get()
        try:
            if item is None:
            ...
            logger.error(f"Failed writing streamed triplet row: {e}")
        finally:
            out_queue.task_done()
...
def compute_support_v2(
    ...
    triplets_queue: Optional["queue.Queue[Optional[Dict[str, Any]]]"] = None,
    keep_support_data: bool = True,
) -> Dict[str, Any]:
    """
    ...
    if status == "SUCCESS" and len(valid_r2) > 0:
        ...
        support_data: Optional[Dict[str, Dict[str, int]]] = {} if keep_support_data else None
            ...
            if use_separate_queries:
                ...
                for pid in chunk:
                    ...
                    if support_data is not None:
                        support_data[pid] = row
                    if triplets_queue is not None:
                        triplets_queue.put({
                            "r1": r1,
                            "r2": pid,
                            "support_loop": loop,
                            "support_nonloop": nonloop,
                            "support_total": total,
                ...
                for pid, counts in parsed.items():
                    ...
                    if triplets_queue is not None:
                        triplets_queue.put({
                            "r1": r1,
                            "r2": pid,
                            "support_loop": row["loop"],
                            "support_nonloop": row["nonloop"],
        ...
        return {
            ...
            "support_data": support_data or {},  # {r2: {loop, nonloop, total}} - only verified r2
            "status": out_status,
            "error": out_error,
            "elapsed_sec": elapsed,
            "updated_at": time.time(),
    ...
    for pid, loop in loop_counts.items():
        ...
        nonloop = max(0, total - loop)
        row = {"loop": loop, "nonloop": nonloop, "total": total}
        if support_data is not None:
            support_data[pid] = row
        if triplets_queue is not None:
            triplets_queue.put({
...
def main() -> None:
    ...
    ap.add_argument(
        "--triplets_output_jsonl",
        default="",
        help="Optional JSONL path for streamed (r1,r2,support_*) rows; written by a dedicated writer thread.",
    )
    ap.add_argument("--triplets_queue_size", type=int, default=10000)
    ...
    triplets_queue: Optional["queue.Queue[Optional[Dict[str, Any]]]"] = None
    triplets_writer: Optional[threading.Thread] = None
    stream_triplets = bool(args.triplets_output_jsonl)
    if stream_triplets:
        triplets_queue = queue.Queue(maxsize=max(1, args.triplets_queue_size))
        triplets_writer = threading.Thread(
            target=triplets_writer_worker,
            args=(args.triplets_output_jsonl, triplets_queue, logger),
            name="triplets-writer",
            daemon=True,
        )
        triplets_writer.start()
        logger.info(f"Streaming triplet rows to {args.triplets_output_jsonl}")
    ...
    try:
        ...
        if triplets_queue is not None:
            triplets_queue.put(None)
            triplets_queue.join()
        if triplets_writer is not None:
            triplets_writer.join(timeout=30)
