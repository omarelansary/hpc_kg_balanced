...
class BalanceAwarePruner:
    ...
    def run(self, state: KGState) -> Tuple[KGState, RunReport]:
        ...
        for round_index in range(1, self.config.max_rounds + 1):
            ...
            if effective_selection_mode == "sequential" and not self.config.dry_run:
                selected, candidate_count, guard_rejections = self._select_round_sequential(
                    state=state,
    ...
    def _select_round_sequential(self, state: KGState, round_budget: int) -> Tuple[List[RemovalDecision], int, int]:
        selected: List[RemovalDecision] = []
        candidate_count = 0
        guard_rejections = 0
        for step_idx in range(round_budget):
            ...
            state.remove_triple(decision.triple_id)
        return selected, candidate_count, guard_rejections
