...
Run state defaults to `outputs/pipeline_runs/`. Use `--state-root PATH` or `KG_PIPELINE_STATE_ROOT=PATH` when a restricted execution context needs to write run state elsewhere; this does not redirect canonical input artifacts or historical graph/data outputs.
...
`replay-frozen` also runs the Phase II run-scoped execution slices now enabled for Stage1 and Stage3. It first writes the existing readiness report, then executes only the historical `score-genericity` and `audit-candidates` subcommands with `--config` and `--run-dir` pointing under `outputs/pipeline_runs/<run_id>/phase2_replay/historical_relation_pipeline_run/`. The run-scoped config rewrites missing historical input paths to frozen archive inputs and forces local candidate-source mode so WDQS cannot be selected by these slices. It writes `stage1_stage3_replay_readiness_report.json`, `stage1_stage3_execution_report.json`, and summaries under `outputs/pipeline_runs/<run_id>/phase2_replay/`.

Stage4 `construct-graph` has a guarded run-scoped wrapper but is long-running because the historical implementation repeatedly rescans frozen candidate shards. It is skipped by default and requires `--execute-stage4` in `replay-frozen`. With that flag, the runner executes only `construct-graph`, writes `stage4_core_graph_execution_report.json`, and keeps Stage5/6/7, Stage11/12 repair, and C1 Stage13 blocked. Stage4 output is only an intermediate run-scoped core graph, not a regenerated B0 endpoint.
...
The state root may be overridden with `--state-root PATH` or `KG_PIPELINE_STATE_ROOT=PATH`. `--status` and `--resume` use the resolved state root unless a specific `--state` file is provided.
