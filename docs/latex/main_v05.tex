...
{\textbf{Branch A: empirical hop-support verification}\\[1.5mm]
\textbullet~symmetry\\
...
\mathrm{ValidR2}(r_1)
\]
This step reduces the theoretical $|R|^2$ space to empirically supported transitions. In the cached discovery artifact used for reconstruction, \texttt{data/processed/hop\_discovery\_from\_json.jsonl}, this stage contains 1,703 first-hop relation records and 56,278 summed valid second-hop relation entries.
...
The first pruning step uses the stored LLM-classified relation-profile artifact as an upstream high-recall semantic triage signal for candidate target relations $r_3$. For each relation, the classifier is given structured metadata describing the relation, including its label, description and other available property metadata collected previously, and is asked only whether the relation is a plausible \emph{target} of a two-hop composition rule of the form
\[
...
To reduce hallucination risk and over-pruning, the prompt was designed conservatively. The model was restricted to a binary decision: either the relation is worth testing as a target or it is \texttt{NO\_WAY}. The \texttt{NO\_WAY} label was intended to be used sparingly and only when testing would be structurally or semantically wasted. When uncertain, the model was instructed to keep the relation rather than discard it. The prompt also explicitly framed composition as a definitional shortcut relation and excluded explanatory, provenance, attribute-propagation, and terminal classification-style relations from positive target decisions. Thus, the LLM-derived labels were used only as a coarse semantic pruning signal with high recall, not as an oracle for logical validity.

In the stored relation-profile artifact, 615 relations were retained as plausible composition targets and 1{,}830 relations were labelled \texttt{NO\_WAY}. These labels are not treated as proof of composition. Instead, they define a reduced target set for downstream compatibility filtering and empirical shortcut verification. This filtering step was applied only to composition target selection and did not affect the candidate sets for symmetry, anti-symmetry, or inversion.

Operationally, the code-compatible relation-triage pipeline operates over JSON relation profiles containing relation identifiers, labels, descriptions, datatypes, domain type IDs, range type IDs, and, when available, bounded example triples and aliases. It appends an \texttt{llm\_classification} object with separate fields for composition, symmetry, anti-symmetry, and inverse suitability. The downstream composition-target enrichment stage uses only \texttt{llm\_classification.composition.composition\_target} and retains every relation whose value is not \texttt{NO\_WAY}. The same response schema also stored symmetry, anti-symmetry, and inverse triage labels, but these auxiliary LLM labels are not treated as the final evidence for symmetry, anti-symmetry, or inversion. In the reconstructed final allocation, those groups are identified empirically in the dashboard from hop-support statistics; the separate inverse-LLM shard workflow was attempted, but completed inverse LLM verification is not evidenced in the preserved artifacts. The preserved relation-profile artifact contains derived labels and reasoning fields, but no raw LLM responses, production-run model metadata, or export manifest were found. Therefore, this step is reported as an upstream pruning/profiling artifact, not as an exactly reproducible LLM production run or as final composition evidence.
...
This compatibility pruning is also applied only to composition and does not modify the candidate sets for symmetry, anti-symmetry, or inversion. In the reconstructed v3 chain, the compatible-target artifact is preserved as a cached intermediate candidate-preparation file. However, the exact v3 domain/range enrichment command and execution log are incomplete, so this step is treated as preserved intermediate evidence rather than a fully replayed export.
...
The first branch uses the hop-support counts directly to identify symmetric, anti-symmetric, and inverse behavior in the dashboard. These labels are empirical pattern scores over retrieved two-hop evidence, not logical Wikidata axioms and not completed LLM inverse classifications.
...
At allocation time, the dashboard converts these candidate-level outputs into relation-level groups. Symmetric and anti-symmetric groups use unique self-pair relations passing the loop or non-loop thresholds, with any overlap retained in the symmetric group and removed from the anti-symmetric group. The inverse group is the unique relation set appearing in accepted ordered inverse candidate pairs. Under the canonical thresholds recorded in the 5k allocation, the reconstructed final pattern sets are reproduced from the patched v3 hop-support artifact and the v3 compact composition-verification artifact with Wilson filtering disabled: 18 symmetric relations, 66 anti-symmetric relations, 44 inverse relations, and 26 composition relations. The exact Streamlit dashboard export session was not preserved, so this is a reconstruction from cached inputs, threshold configuration, and matching exported relation sets rather than a saved-session replay.
...
Thus, composition confidence is an empirical shortcut frequency over sampled verified chains, not a direct function of the phase-1 loop/non-loop support counts. In the dashboard, this quantity is loaded from the compact composition-verification output and may optionally be refined by Wilson-score lower-bound filtering before candidate acceptance. Final composition evidence therefore comes from sampled shortcut verification and dashboard thresholds, not from the upstream LLM labels alone. For the canonical 5k allocation used downstream, the reconstruction evidence does not support treating Wilson filtering as part of the final allocation selection: the final composition group is reproduced from the v3 compact composition-verification output under the canonical dashboard thresholds with Wilson filtering disabled.
...
The provenance of the final exported allocation must be stated carefully. The checked-in dashboard defaults point to v2 hop-support and v2 compact composition-verification artifacts, but those defaults do not reproduce the canonical 5k allocation relation universe. The canonical 5k allocation is most consistent with the patched v3 hop-support artifact and the v3 compact composition-verification artifact, with Wilson filtering disabled; however, the exact dashboard export session was not preserved. This distinction separates confirmed cached artifacts from the evidence-based inference about the final dashboard inputs.
...
In summary, the first phase of the methodology transitions from a theoretical, unconstrained relation universe to a concrete, empirically verified foundation. By discovering observed paths, pruning unviable targets, and verifying structural patterns, the pipeline distills the data into two actionable outputs: an integer target quota ($\eta_r$) for each accepted relation and an exported relation-level support matrix. The final 5k allocation and support-matrix artifacts are hashable and are used downstream, but exact dashboard export provenance remains incomplete.
